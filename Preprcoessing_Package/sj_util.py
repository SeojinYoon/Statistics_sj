...
def find_indexes(list, find_value):
    indexes = []
...
def get_random_sample_in_codes(sample_count, codes, ratio):
    """
    ...
    :param ratio: code's ratio per element ex) [0.2, 0.3, 0.5]
    ...
    for code_i in range(0, len(codes)):
        lottery_count = int(sample_count * ratio[code_i])

        if code_i != len(codes) - 1:
            for j in range(0, lottery_count):
...
if __name__ == "__main__":
    ...
    get_random_sample_in_codes(10, [1,2,3], [0.2, 0.2, 0.8])
